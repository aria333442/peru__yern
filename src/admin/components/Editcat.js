...
import { createcat } from "../actions/createcategory";
import { createsubcat } from "../actions/createesubcat";
import Adminnv from "../components/Adminnav";
...
function Editcat() {
      ...
      .then((data) => {
        if (data.url) {
          if (!name || !tags || !data.url || !slug) {
            ...
            dispatch(createcat(item)).then((res) => {
              setname("");
              settags("");
              setslug("");
              setimage("");
  ...
  const addsubcategory = () => {
    if (!subname || !category) {
      ...
      dispatch(createsubcat(item)).then((res) => {
        setsubname("");
        setcategory("");
        ...
        <div className="encatright">
          ...
          <div className="encatmini">
            <div className="encatminileft">
              ...
              <div
                className="encatlefth"
                style={{ marginTop: "20px", marginBottm: "10px" }}
              >
                <p style={{ textTransform: "uppercase" }}>
                  Selecciona una categoría
                </p>
              </div>
              <div className="henana">
                <Select options={options} onChange={setcategory}></Select>
              </div>
              <div
                className="encataggr"
                style={{ marginTop: "20px !important" }}
              >
                <p className="addcateggory" onClick={addsubcategory}>
                  AGREGAR
                </p>
              </div>
            </div>
