...
import { deletenew } from "../actions/deletenew";

function Allhomeproduct() {
  ...
  const pageCount = Math.ceil(getnewP.product.length / worksperpage);
  const changePage = ({ selected }) => {
    setpagenumber(selected);
  };
  const dispalyblogs = getnewP.product
    ...
    .map((blog) => {
      return (
        <tr className="blgdettr2">
          ...
          <td className="blgdetstd2">
            <div style={{ width: "100px", height: "100px" }}>
              <img
                src={blog.productImage[0].img}
                style={{ width: "100%", height: "100%" }}
              ></img>
            </div>
          </td>
          <td className="blgdetttd">{blog.name}</td>
          <td className="blgdetfftd">{blog.description.substr(1, 50)}</td>
          <td className="blgdetfftd">{`/product/${blog._id}`}</td>
        </tr>
      );
    });
  const editit = () => {
    if (all.length > 0) {
      ...
      const Nid = checked[0];
      history.push(`/admin/editnewhome/${Nid}`);
    }
  };
  const deleteit = () => {
    if (all.length > 0) {
      ...
      dispatch(deletenew(item)).then((res) => {
              ...
              <div className="sltfmsub">
                ...
                <div className="sltfmsubr">
                  <div className="sltfmsubrf">
                    <NavLink to="/admin/homenewproduct" className="haiga">
                      <p>
                        <IoMdAddCircle />
                      </p>
                    </NavLink>
                  ...
                  <div className="sltfmsubrs">
                    <p className="sltfmsubrsp">Nuevo productos</p>
                  </div>
                </div>
              </div>
